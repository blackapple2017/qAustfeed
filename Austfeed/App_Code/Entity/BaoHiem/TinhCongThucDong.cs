...
/// <summary>
/// Summary description for TinhCongThucDong
/// </summary>
public class TinhCongThucDong
{
    ...
    public class Function
    {
                ...
                switch (state)
                {
                    ...
                    case 2:
                        ...
                        else if (!Char.IsLetter(equation[i]))
                        {
                          ...
                          decimal itme1=  decimal.Parse(temp.ToString(), new NumberFormatInfo() { NumberDecimalSeparator = "." });
                            ctSymbol.m_value = itme1;
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (ctSymbol.m_name == "pi")
                                    ctSymbol.m_value = Convert.ToDecimal(System.Math.PI);
                                else if (ctSymbol.m_name == "e")
                                    ctSymbol.m_value = Convert.ToDecimal(System.Math.E);
        ...
        protected Symbol Evaluate(Symbol sym1, Symbol opr, Symbol sym2)
        {
            ...
            result.m_value = 0;
            decimal a = sym1.m_value;
            decimal b = sym2.m_value;
            switch (opr.m_name)
            {
                case "^":
                ...
                case "/":
                    {
                        if (sym2.m_value > 0)
                            result.m_value = Convert.ToDecimal(a/b);
                        else
                        {
    ...
    //#region Xử lý tính toán công thức
    //public int MangChuoiToSo(string[] saSo)
    //{
    //    string sAllString = null;
    //    foreach (var item in saSo)
    //    {
    //        sAllString += item;
    //    }
    //    return int.Parse(sAllString);
    //}
    ...
    //        else// IsOperator(s)
    //        {
    //            while (stack.Count > 0 && GetPriority(s) <= GetPriority(stack.Peek().ToString()))
